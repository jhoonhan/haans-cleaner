export const LOADING_TOGGLE_ACTION = "LOADING_TOGGLE_ACTION";

export const SIGN_IN = "SIGN_IN";
export const SIGN_OUT = "SIGN_OUT";

export const FETCH_ORDER = "FETCH_ORDER";
export const FETCH_ORDERS = "FETCH_ORDERS";
export const CREATE_ORDER = "CREATE_ORDER";
export const EDIT_ORDER = "EDIT_ORDER";
export const CANCEL_ORDER = "CANCEL_ORDER";

export const FETCH_USER = "FETCH_USER";
export const LOGIN_USER = "LOGIN_USER";
export const FETCH_USERS = "FETCH_USERS";
export const CREATE_USER = "CREATE_USER";
export const EDIT_USER = "EDIT_USER";
export const DELETE_USER = "DELETE_USER";
export const MOUNT_USER = "MOUNT_USER";
export const CANCEL_USER_ORDER = "CANCEL_USER_ORDER";

export const EDIT_DADDRESS = "EDIT_DADDRESS";

export const FETCH_COORDS = "FETCH_COORDS";

export const D_FETCH_ORDER = "D_FETCH_ORDER";
export const D_ACCEPT_ORDER = "D_ACCEPT_ORDER";
export const D_EDIT_ORDER = "D_EDIT_ACCEPTED_ORDER";
export const D_EDIT_ACCEPTED_ORDER = "D_EDIT_ACCEPTED_ORDER";
export const D_CANCEL_ORDER = "D_CANCEL_ORDER";
export const D_GET_COORDS = "D_GET_COORDS";
export const D_SET_COORDS = "D_SET_COORDS";
export const D_FETCH_ACCEPTED = "D_FETCH_ACCEPTED";
export const D_SET_DISTANCE = "D_SET_DISTANCE";
export const D_SET_GEOCODE = "D_SET_GEOCODE";
export const D_COMPLETE_ORDER = "D_COMPLETE_ORDER";
export const D_CLEAR_ORDER = "D_CLEAR_ORDER";

export const ERROR_HTTP = "ERROR_HTTP";
