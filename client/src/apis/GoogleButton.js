...
class GoogleButton extends React.Component {
  ...
  renderAuthButton() {
    ...
    if (!this.props.isSignedIn) {
      return (
        <div onClick={() => this.onSignInClick(true)}>Sign in with Google</div>
      );
    }
  }
  render() {
    return (
      <div className="login-with-google-btn">{this.renderAuthButton()}</div>
    );
  }
}
...
export default connect(mapStateToProps, { signIn })(GoogleButton);
